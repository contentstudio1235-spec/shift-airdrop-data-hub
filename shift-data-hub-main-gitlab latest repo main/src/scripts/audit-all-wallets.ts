  ...
  try {
    ...
    for (const user of usersResult.rows) {
      ...
      const posResult = await pool.query(
        `SELECT asset, position_size_usd, opened_at, daysHeld
         FROM positions
    ...
    if (needsBackfillCount > 0) {
      ...
      auditResults.slice(0, 20).forEach((audit) => {
        console.log(
          `${audit.wallet.slice(0, 20)}... | ${audit.positionCount} | ${audit.zeroUsdCount} | $${audit.totalPositionValue.toFixed(2).padStart(7)} | ${audit.expectedXpMinimum.toFixed(0).padStart(4)} | ${audit.currentXp.toFixed(0).padStart(4)} | ${audit.xpDeficit.toFixed(0)}`
        );
      });
