...
async function estimateTokenPrices(): Promise<Map<string, TokenEstimate>> {
...
async function backfillSubdollarPositions(estimates: Map<string, TokenEstimate>): Promise<number> {
  ...
  for (const [asset, estimate] of estimates) {
    ...
    const result = await pool.query(
      `SELECT id, wallet, token_amount, position_size_usd
       FROM positions
...
async function recalculateAffectedWalletXP(wallets: Set<string>): Promise<number> {
    ...
    await pool.query(
      `UPDATE positions 
       ...
       WHERE wallet = $1 AND status = 'open'
         AND asset IN ('TSL2L','TSL1S','SOX3L','SOX3S','SPX3L','SPX3S')`,
      [wallet]
    );

    // Zero out user XP so it recalculates from scratch
    await pool.query(
      `UPDATE users SET total_xp = 0 WHERE wallet = $1`,
      [wallet]
    );
