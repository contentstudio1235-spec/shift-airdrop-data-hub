...
import type { HeliusWebhookPayload, HeliusSwapEvent } from '../types';

interface SwapPriceData {
  tokenAmount: number;         // Tokens received (with decimals applied)
  stablecoinAmount: number;    // USDC/USDT paid (with decimals applied)
  pricePerToken: number;       // Calculated: stablecoin / token
  timestamp: Date;
  txSignature: string;
...
interface HeliusTransactionDetails {
  type?: string;
  nativeTransfers?: any[];
  tokenTransfers?: any[];
  events?: {
...
async function fetchTransactionFromHelius(
  txSignature: string
): Promise<HeliusTransactionDetails | null> {
  const primaryKey = config.heliusApiKey;
  ...
  if (primaryKey) {
    try {
      const response = await axios.get<any[]>(
        `https://api.helius.xyz/v0/transactions?txs=${txSignature}&api-key=${primaryKey}`,
        { timeout: 10000 }
      );
      ...
      if (axios.isAxiosError(primaryErr)) {
      ...
      if (backupKey) {
        try {
          const backupResponse = await axios.get<any[]>(
            `https://api.helius.xyz/v0/transactions?txs=${txSignature}&api-key=${backupKey}`,
            { timeout: 10000 }
          );
          ...
          if (axios.isAxiosError(backupErr)) {
 ...
 */
function extractPriceFromSwapEvent(
  swapEvent: HeliusSwapEvent,
  wallet: string,
  assetMint: string
): SwapPriceData | null {
  ...
  const tokenOutput = swapEvent.tokenOutputs?.find(
    (out) => out.mint === assetMint && out.userAccount === wallet
  );

  if (!tokenOutput) return null;

  // Find the stablecoin input
  const stablecoinInput = swapEvent.tokenInputs?.find(
    (inp) =>
      STABLE_COIN_MINTS.includes(inp.mint) && inp.userAccount === wallet
  );

  if (!stablecoinInput) return null;
  ...
  const tokenAmount =
    parseFloat(tokenOutput.rawTokenAmount?.tokenAmount || '0') /
    Math.pow(10, tokenDecimals);
  const stablecoinAmount =
    parseFloat(stablecoinInput.rawTokenAmount?.tokenAmount || '0') /
    Math.pow(10, stablecoinDecimals);

  if (tokenAmount === 0 || stablecoinAmount === 0) return null;
 ...
 */
export async function extractSwapPriceFromTx(
  txSignature: string,
  wallet: string,
  assetMint: string
): Promise<SwapPriceData | null> {
  console.log(
    `[Helius] Extracting price for tx ${txSignature.slice(0, 20)}... asset ${assetMint.slice(0, 12)}...`
  );
  ...
  if (!txData) {
    console.warn(
      `[Helius] Could not fetch transaction data for ${txSignature}`
    );
    return null;
  }
  ...
  if (txData.events?.swap) {
    const priceData = extractPriceFromSwapEvent(
      txData.events.swap,
      wallet,
      assetMint
    );
    if (priceData) {
      ...
      console.log(
        `[Helius] ✅ Extracted: ${priceData.pricePerToken.toFixed(4)} per token`
      );
      return priceData;
    }
  }
 ...
 */
export async function batchExtractSwapPrices(
  positions: Array<{
    id: string;
    wallet: string;
    asset_mint: string;
  ...
  const results = new Map<string, SwapPriceData>();

  const CHUNK_SIZE = 10;
  const DELAY_MS = 1000; // 1s between chunks

  console.log(
    `[Helius] Batch processing ${positions.length} positions (${Math.ceil(positions.length / CHUNK_SIZE)} chunks)...`
  );
  ...
  for (let i = 0; i < positions.length; i += CHUNK_SIZE) {
    ...
    const promises = chunk.map((pos) =>
      extractSwapPriceFromTx(pos.tx_signature_open, pos.wallet, pos.asset_mint)
        .then((priceData) => {
          if (priceData) {
    ...
    if (i + CHUNK_SIZE < positions.length) {
      console.log(
        `[Helius] Processed ${Math.min(i + CHUNK_SIZE, positions.length)}/${positions.length} positions, waiting ${DELAY_MS}ms...`
      );
  ...
  console.log(
    `[Helius] ✅ Batch complete: extracted ${results.size}/${positions.length} prices`
  );
  return results;
}
 ...
 */
export async function verifyTransactionSwap(txSignature: string): Promise<any> {
  const txData = await fetchTransactionFromHelius(txSignature);
  if (!txData) {
