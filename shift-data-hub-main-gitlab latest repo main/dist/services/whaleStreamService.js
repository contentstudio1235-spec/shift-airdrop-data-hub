...
exports.fetchInitialWhales = fetchInitialWhales;
exports.pollNewWhales = pollNewWhales;
// src/services/whaleStreamService.ts
const pool_1 = require("../db/pool");
const SNAG_PATTERN = /^[a-zA-Z0-9]{6}$/;
function classifySource(firstUtm, referredBy) {
    const utm = firstUtm && firstUtm.length > 0 && firstUtm !== 'unknown_legacy' ? firstUtm.toLowerCase() : null;
    if (utm && SNAG_PATTERN.test(utm))
        return 'snag_referrals';
    if (referredBy && SNAG_PATTERN.test(referredBy))
        return 'snag_referrals';
...
const WHALE_SQL = `
  ...
  WHERE pos.position_size_usd >= 1000
    AND pos.status = 'open'
`;
function rowToEvent(r, isHistorical) {
    return {
        type: 'open',
        wallet: r.wallet,
...
async function fetchInitialWhales(limit = 20) {
    const rows = await (0, pool_1.query)(`${WHALE_SQL} ORDER BY pos.opened_at DESC LIMIT $1`, [limit]);
    return rows.map(r => rowToEvent(r, true));
}
async function pollNewWhales(since) {
    const rows = await (0, pool_1.query)(`${WHALE_SQL} AND pos.opened_at > $1 ORDER BY pos.opened_at ASC LIMIT 50`, [since.toISOString()]);
    return rows.map(r => rowToEvent(r, false));
}
//# sourceMappingURL=whaleStreamService.js.map