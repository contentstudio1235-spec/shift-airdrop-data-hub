...
exports.referralTrackingService = exports.ReferralTrackingService = void 0;
const pool_1 = require("../db/pool");
class ReferralTrackingService {
    /**
     * Get referrer's referral stats
     */
    async getReferrerStats(wallet) {
        try {
         ...
         WHERE referrer_wallet = $1`, [wallet]);
            ...
            const activeResult = await (0, pool_1.queryOne)(`SELECT COUNT(*) as active_count
         FROM referrals r
         JOIN users u ON r.referred_wallet = u.wallet
         WHERE r.referrer_wallet = $1 AND u.total_xp > 0`, [wallet]);
     ...
     */
    async getReferredUsers(wallet, limit = 20) {
        try {
    ...
    async getMyReferrer(wallet) {
        try {
         ...
         WHERE r.referred_wallet = $1`, [wallet]);
            if (!result) {
                return null;
            }
            return {
     ...
     */
    async getNetworkXp(wallet) {
        try {
            // My XP
            const myResult = await (0, pool_1.queryOne)(`SELECT total_xp FROM users WHERE wallet = $1`, [wallet]);
            const myXp = parseInt(myResult?.total_xp || '0', 10);
            // Referral XP sum
            const refResult = await (0, pool_1.queryOne)(`SELECT COALESCE(SUM(u.total_xp), 0) as total
         FROM referrals r
         JOIN users u ON r.referred_wallet = u.wallet
         WHERE r.referrer_wallet = $1`, [wallet]);
...
exports.referralTrackingService = new ReferralTrackingService();
//# sourceMappingURL=referralTrackingService.js.map