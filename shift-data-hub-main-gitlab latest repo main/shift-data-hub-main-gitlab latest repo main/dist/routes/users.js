...
router.get('/search', async (req, res) => {
    try {
        ...
        const result = await (0, identityService_1.searchProfiles)({ q, page: 1, pageSize: 25 });
...
router.get('/:profileId', async (req, res) => {
    try {
        ...
        const profile = await (0, identityService_1.getProfile)(profileId);
        if (!profile)
...
router.get('/:profileId/timeline', async (req, res) => {
    try {
        const profileId = String(req.params.profileId);
        const entries = await (0, identityService_1.getTimeline)(profileId, {
...
router.get('/:profileId/positions', async (req, res) => {
    try {
        const profileId = String(req.params.profileId);
...
router.post('/:profileId/links', async (req, res) => {
    try {
        ...
        const conf = VALID_CONFIDENCE.includes(confidence) ? confidence : 'manual';
        const link = await (0, identityService_1.linkIdentity)(profileId, type, value, conf, {
            byActor: 'admin',
...
router.delete('/:profileId/links/:linkId', async (req, res) => {
    try {
        ...
        await (0, identityService_1.unlinkIdentity)(linkId, reason, 'admin');
...
router.post('/:profileId/merge', async (req, res) => {
    try {
        const profileId = String(req.params.profileId);
        ...
        const winner = await (0, identityService_1.mergeProfiles)(profileId, loserProfileId, {
            byActor: 'admin',
            reason,
        });
        res.json(winner);
        ...
        if (err instanceof identity_1.ProfileNotFoundError) {
            return res.status(404).json({ error: 'profile_not_found', profileId: err.profileId });
        }
        if (err instanceof identity_1.MergeWithoutEvidenceError) {
...
router.patch('/:profileId/primary-wallet', async (req, res) => {
    try {
        const profileId = String(req.params.profileId);
        ...
        const linked = await (0, pool_1.query)(`
      ...
      `, [profileId, wallet]);
        ...
        await (0, pool_1.query)(`UPDATE user_profiles SET primary_wallet = $1, updated_at = NOW() WHERE profile_id = $2`, [wallet, profileId]);
        const profile = await (0, identityService_1.getProfile)(profileId);
        res.json(profile);
    }
    catch (err) {
...
router.post('/:profileId/forget', async (req, res) => {
    try {
        const profileId = String(req.params.profileId);
