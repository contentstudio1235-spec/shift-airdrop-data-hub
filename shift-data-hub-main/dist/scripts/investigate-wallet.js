...
async function investigateWallet() {
    ...
    try {
        // Get all positions
        const result = await pool.query(`SELECT id, asset, position_size_usd, opened_at, closed_at, status, tx_signature_open
       FROM positions
       WHERE wallet = $1
       ORDER BY opened_at DESC`, [wallet]);
        ...
        const userResult = await pool.query(`SELECT total_xp, claim_multiplier, created_at FROM users WHERE wallet = $1`, [wallet]);
...
investigateWallet();
//# sourceMappingURL=investigate-wallet.js.map