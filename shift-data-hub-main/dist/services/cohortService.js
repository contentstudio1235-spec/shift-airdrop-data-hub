...
exports._clearCohortSnapshotCache = _clearCohortSnapshotCache;
exports.getCohortsSnapshot = getCohortsSnapshot;
...
function _clearCohortSnapshotCache() {
    snapshotCache.clear();
}
...
function mapCohortEntry(row) {
    ...
    const retentionWeek1 = ageDays >= 14 ? (size === 0 ? 0 : round1((retainedW1 / size) * 100)) : null;
    const retentionWeek4 = ageDays >= 35 ? (size === 0 ? 0 : round1((retainedW4 / size) * 100)) : null;
    const weekStartIso = row.week_start instanceof Date
...
function buildSummary(cohorts) {
    ...
    let trend;
    if (deltaPP > TREND_DELTA_THRESHOLD_PP)
        trend = 'improving';
    else if (deltaPP < -TREND_DELTA_THRESHOLD_PP)
        trend = 'declining';
    else
        trend = 'stable';
    ...
    for (const c of cohorts) {
        if (c.activationPct > best.activationPct)
            best = c;
        if (c.activationPct < worst.activationPct)
            worst = c;
    }
    return {
        trend,
 ...
 */
async function getCohortsSnapshot() {
    const now = Date.now();
