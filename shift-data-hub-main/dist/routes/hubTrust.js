...
const hubTelemetryService_1 = require("../services/hubTelemetryService");
const hubFlagsService_1 = require("../services/hubFlagsService");
const hubIncidentsService_1 = require("../services/hubIncidentsService");
...
router.post('/sessions', async (req, res) => {
    try {
        ...
        await (0, hubTelemetryService_1.startSession)({
            sessionId,
...
router.post('/events', async (req, res) => {
    try {
        ...
        if (!(0, hubTelemetryService_1.isHubEventType)(body.eventType)) {
            return res.status(400).json({ error: 'eventType_invalid' });
        }
        const metadata = body.metadata && typeof body.metadata === 'object' && !Array.isArray(body.metadata)
            ? body.metadata
            : null;
        const recorded = await (0, hubTelemetryService_1.recordEvent)({
            sessionId,
...
router.post('/sessions/:id/close', async (req, res) => {
    try {
        ...
        await (0, hubTelemetryService_1.closeSession)(sessionId);
        res.json({ ok: true });
    }
    catch (err) {
...
router.post('/flags', async (req, res) => {
    try {
        ...
        const flag = await (0, hubFlagsService_1.fileFlag)({
            sessionId,
            hubRole: asOptStr(body.hubRole),
            tab,
            metricId,
...
router.get('/flags', async (req, res) => {
    try {
        ...
        const status = statusRaw && (0, hubFlagsService_1.isHubFlagStatus)(statusRaw) ? statusRaw : null;
        ...
        const flags = await (0, hubFlagsService_1.listFlags)({ status, sinceISO, limit });
        res.json({ flags });
    }
    catch (err) {
...
router.patch('/flags/:id', async (req, res) => {
    try {
        ...
        const body = asObject(req.body);
        if (!(0, hubFlagsService_1.isHubFlagTriageStatus)(body.status)) {
        ...
        const updated = await (0, hubFlagsService_1.triageFlag)(id, {
            status: body.status,
            triagedBy,
...
router.get('/incidents/active', async (_req, res) => {
    try {
        const incidents = await (0, hubIncidentsService_1.listActive)();
        res.json({ incidents });
    }
    catch (err) {
...
router.post('/incidents', async (req, res) => {
    try {
        ...
        if (!(0, hubIncidentsService_1.isHubIncidentSeverity)(body.severity)) {
        ...
        const incident = await (0, hubIncidentsService_1.openIncident)({
            openedBy,
...
router.patch('/incidents/:id/resolve', async (req, res) => {
    try {
        ...
        const body = asObject(req.body);
        if (!(0, hubIncidentsService_1.isHubIncidentResolution)(body.resolution)) {
            return res.status(400).json({ error: 'resolution_invalid' });
        }
        const updated = await (0, hubIncidentsService_1.resolveIncident)(id, {
...
//# sourceMappingURL=hubTrust.js.map