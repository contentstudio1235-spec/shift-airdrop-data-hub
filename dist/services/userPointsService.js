...
exports.userPointsService = exports.UserPointsService = void 0;
const pool_1 = require("../db/pool");
class UserPointsService {
    static POSITION_MULTIPLIER = 2.0;
    static SOCIAL_MULTIPLIER = 1.0;
    static REFERRAL_POSITION_MULTIPLIER = 1.0;
    async calculateFinalPoints(wallet) {
       ...
       FROM users WHERE wallet = $1`, [wallet]);
        if (!row)
            return { wallet, positionSp: 0, socialSp: 0, referralPositionSp: 0, finalPoints: 0 };
        const finalPoints = Number(row.total_xp) * UserPointsService.POSITION_MULTIPLIER +
    ...
    async calculateAllFinalPoints(limit = 10000) {
    ...
    async getRankByFinalPoints(wallet) {
        const result = await (0, pool_1.queryOne)(`WITH ranked AS (
         SELECT wallet,
       ...
       SELECT rank FROM ranked WHERE wallet = $1`, [wallet]);
    ...
    async getTopByFinalPoints(limit = 100) {
       ...
       SELECT * FROM ranked LIMIT $1`, [limit]);
        return results.rows.map(row => ({
            wallet: row.wallet,
            positionSp: Math.floor(Number(row.total_xp)),
            socialSp: Math.floor(Number(row.snag_points)),
...
exports.userPointsService = new UserPointsService();
//# sourceMappingURL=userPointsService.js.map