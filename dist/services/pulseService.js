...
exports._clearPulseCache = _clearPulseCache;
exports.getPulseSnapshot = getPulseSnapshot;
...
function pctChange(delta, prior, decimals = 2) {
 ...
 */
async function computeRegisteredUsers() {
 ...
 */
async function computeActiveHolders() {
    const row = await (0, pool_1.queryOne)(`WITH open_wallets AS (
 ...
 */
async function computeStitchPct() {
    const row = await (0, pool_1.queryOne)(`SELECT (
       (SELECT COUNT(*) FROM (
          SELECT profile_id
            FROM identity_links
 ...
 */
async function computeActivations24h() {
    const row = await (0, pool_1.queryOne)(`SELECT COUNT(*)::bigint AS activations
       FROM (
         SELECT wallet, MIN(opened_at) AS first_opened_at
           FROM positions
          GROUP BY wallet
 ...
 */
async function computeOpenWhalesCount() {
    const row = await (0, pool_1.queryOne)(`SELECT
       ...
       (SELECT COUNT(*) FROM positions
         WHERE closed_at  > NOW() - INTERVAL '24 hours'
           AND position_size_usd >= $1)::bigint AS closes`, [WHALE_USD_THRESHOLD]);
    const total = toNumber(row?.total);
 ...
 */
async function computeAum30d() {
    const rows = await (0, pool_1.query)(`WITH days AS (
 ...
 */
async function computeSignupsBySource24h() {
    const rows = await (0, pool_1.query)(`SELECT COALESCE(first_utm_source, 'direct') AS source,
    ...
    if (otherCount > 0)
        kept.push({ source: 'other', count: otherCount });
 ...
 */
async function computeWhaleActivity24h() {
    const rows = await (0, pool_1.query)(`(SELECT opened_at AS occurred_at, wallet, asset,
     ...
     LIMIT $2`, [WHALE_USD_THRESHOLD, WHALE_FEED_LIMIT]);
    return rows.map(r => ({
 ...
 */
async function getStitchBaseline() {
    try {
        const row = await (0, pool_1.queryOne)(`SELECT setting_value FROM airdrop_config WHERE setting_key = $1`, ['pulse_stitch_baseline']);
        if (!row)
 ...
 */
async function computeAnomalies(stitchPct, whaleCount, signups) {
    const out = [];
    // 1. Stitch drop
...
async function getPulseSnapshot() {
    ...
    const [registeredUsers, activeHolders, aumUSD, stitchPct, activations24h, openWhalesCount, aum30d, signupsBySource24h, whaleActivity24h,] = await Promise.all([
    ...
    const anomalies = await computeAnomalies(stitchPct.value, whaleActivity24h.length, signupsBySource24h);
    const snapshot = {
