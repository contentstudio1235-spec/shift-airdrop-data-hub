...
exports.snagSyncService = exports.SnagSyncService = void 0;
...
class SnagSyncService {
    ...
    constructor() {
        this.client = axios_1.default.create({
            ...
            headers: {
                'x-api-key': config_1.config.snagApiKey,
    ...
    recordFailure() {
        ...
        if (this.circuitFailures >= CIRCUIT_THRESHOLD && !this.circuitOpen) {
            ...
            console.error(`[SnagSync] 🔴 CIRCUIT BREAKER OPEN — SNAG failed ${this.circuitFailures}× ` +
    ...
    async queueFailedEntries(entries, syncType, errorMessage, batchId) {
        const backoffUntil = new Date(Date.now() + this.backoffMs(this.circuitFailures));
        for (const entry of entries) {
            try {
                await (0, pool_1.execute)(`INSERT INTO snag_sync_queue
           ...
           ON CONFLICT DO NOTHING`, [entry.wallet, syncType, JSON.stringify(entry), batchId, backoffUntil, errorMessage.slice(0, 512)]);
            }
            catch (dbErr) {
    ...
    async fullSync() {
        ...
        try {
            ...
            await multiplierService_1.multiplierService.recalculateAllMultipliers();
            // 3. Evaluate badges for all users
            const newBadges = await badgeService_1.badgeService.evaluateAllUsers();
    ...
    // ── XP Sync (Batched) ──
    async syncAllXP() {
        const users = await (0, pool_1.query)(`SELECT u.wallet, u.total_xp, u.snag_user_id,
       ...
       LEFT JOIN xp_sync_log s ON u.wallet = s.wallet`);
        ...
        for (const wallet of succeeded) {
           ...
           VALUES ($1, $2, NOW())
           ON CONFLICT (wallet) DO UPDATE SET last_synced_xp = $2, last_synced_at = NOW()`, [wallet, entry.currentXP]);
            }
            catch (err) {
    ...
    async batchPushXP(entries) {
        ...
        if (!config_1.config.snagLoyaltyCurrencyId) {
            // No loyalty currency configured — skip XP sync
            if (!config_1.config.snagXpRuleId) {
        ...
        for (let i = 0; i < entries.length; i += BATCH_SIZE) {
            ...
            try {
                await this.client.post('/api/loyalty/transactions', {
                    ...
                    organizationId: config_1.config.snagOrganizationId, // Some SNAG versions require this
     ...
     */
    async pushXPViaRule(wallet, xpAmount) {
        if (!config_1.config.snagXpRuleId)
            return;
        await this.client.post(`/api/loyalty/rules/${config_1.config.snagXpRuleId}/complete`, {
     ...
     */
    async syncMultipliers() {
        if (!this.checkCircuit() || !config_1.config.snagWebsiteId || !config_1.config.snagLoyaltyCurrencyId) {
            if (!config_1.config.snagLoyaltyCurrencyId) {
                console.log('[SnagSync] Multiplier sync skipped: SNAG_LOYALTY_CURRENCY_ID not configured');
            }
            return;
        }
        const users = await (0, pool_1.query)(`SELECT wallet, claim_multiplier, snag_multiplier_id
       FROM users
            ...
            try {
                ...
                if (user.snag_multiplier_id) {
                    ...
                    if (multiplierIdCreated) {
                        await (0, pool_1.execute)('UPDATE users SET snag_multiplier_id = $1 WHERE wallet = $2', [multiplierIdCreated, user.wallet]);
                    }
                }
                this.recordSuccess();
                synced++;
    ...
    // ── Badge Sync ──
    async awardBadgeInSnag(wallet, badgeName) {
        const snagBadgeId = config_1.config.snagBadgeIds[badgeName];
        ...
        try {
            ...
            await (0, pool_1.execute)('UPDATE badges SET snag_badge_id = $1 WHERE wallet = $2 AND badge_name = $3', [snagBadgeId, wallet, badgeName]);
            this.recordSuccess();
     ...
     */
    async awardSocialRule(wallet, ruleId, points) {
        if (!ruleId || !this.checkCircuit()) {
            if (!ruleId)
                console.log('[SnagSync] No rule ID supplied, skipping social rule award');
            return;
        }
        try {
            ...
            await this.queueFailedEntries([{ wallet, rule_id: ruleId, points }], 'social_rule', errMsg, `social-${ruleId}`);
        }
    }
    // ── Queue Retry ──
    /**
    ...
    async processRetryQueue() {
        if (!this.checkCircuit())
            return;
        const items = await (0, pool_1.query)(`SELECT id, wallet, sync_type, payload, attempts
       ...
       LIMIT 50`);
        ...
        await (0, pool_1.execute)(`UPDATE snag_sync_queue SET status = 'abandoned', attempts = 10
       WHERE status = 'failed'
        ...
        if (xpItems.length > 0) {
            ...
            for (const entry of entries) {
                if (succeededSet.has(entry.wallet)) {
                    await (0, pool_1.execute)(`UPDATE snag_sync_queue SET status = 'completed', last_attempt_at = NOW() WHERE id = $1`, [entry.queueId]);
                }
                else {
                    const nextBackoff = new Date(Date.now() + this.backoffMs(entry.attempts));
                    await (0, pool_1.execute)(`UPDATE snag_sync_queue
             SET attempts = attempts + 1, last_attempt_at = NOW(), backoff_until = $1
             WHERE id = $2`, [nextBackoff, entry.queueId]);
                }
            }
        }
        ...
        for (const item of badgeItems) {
            try {
                ...
                await this.awardBadgeInSnag(item.wallet, p.badge_name);
                await (0, pool_1.execute)(`UPDATE snag_sync_queue SET status = 'completed', last_attempt_at = NOW() WHERE id = $1`, [item.id]);
            }
            catch {
                const nextBackoff = new Date(Date.now() + this.backoffMs(item.attempts));
                await (0, pool_1.execute)(`UPDATE snag_sync_queue
           SET attempts = attempts + 1, last_attempt_at = NOW(), backoff_until = $1
           WHERE id = $2`, [nextBackoff, item.id]);
            }
        }
    }
    // ── Leaderboard ──
    ...
    async getLocalLeaderboard(limit) {
    ...
    async getUserRank(wallet) {
        if (config_1.config.snagApiKey && config_1.config.snagWebsiteId) {
            try {
                const user = await (0, pool_1.queryOne)('SELECT snag_user_id FROM users WHERE wallet = $1', [wallet]);
        ...
        const result = await (0, pool_1.queryOne)(`SELECT COUNT(*) + 1 as rank FROM users WHERE total_xp > (SELECT total_xp FROM users WHERE wallet = $1)`, [wallet]);
        return result ? parseInt(result.rank, 10) : null;
    }
    async getUserPoints(wallet) {
        if (!config_1.config.snagApiKey || !config_1.config.snagWebsiteId)
            return 0;
        try {
     ...
     */
    async getUserReferralLinks(wallet) {
        if (!config_1.config.snagApiKey || !config_1.config.snagWebsiteId) {
        ...
        try {
            ...
            const response = await this.client.get(`/api/loyalty/accounts/${snagUserId}/referral-links`, { params: { websiteId: config_1.config.snagWebsiteId } });
         ...
         WHERE wallet = $3`, [defaultLink, customLink, wallet]);
            this.recordSuccess();
     ...
     */
    async setCustomReferralLink(wallet, customCode) {
        if (!config_1.config.snagApiKey || !config_1.config.snagWebsiteId) {
        ...
        try {
            ...
            await this.client.patch(`/api/loyalty/accounts/${user.snag_user_id}/referral-links`, { customCode }, { params: { websiteId: config_1.config.snagWebsiteId } });
            // Cache in local DB
            await (0, pool_1.execute)(`UPDATE users
         SET snag_custom_referral_code = $1
         WHERE wallet = $2`, [customCode, wallet]);
            this.recordSuccess();
...
exports.SnagSyncService = SnagSyncService;
exports.snagSyncService = new SnagSyncService();
//# sourceMappingURL=snagSyncService.js.map