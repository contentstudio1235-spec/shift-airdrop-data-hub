...
exports.positionPriceService = exports.PositionPriceService = exports.RWA_TOKEN_MINTS = void 0;
...
class PositionPriceService {
    // ─────────────────────────────────────────────────────────────────────────
    // ENTRY: Store price + amount when a position is OPENED
    // Called from positionService.openPosition()
    // ─────────────────────────────────────────────────────────────────────────
    async recordEntryPrice(positionId, assetMint, tokenAmount) {
        try {
            const price = await jupiterPriceService_1.jupiterPriceService.getPrice(assetMint);
            if (!price) {
         ...
         SET entry_price       = $1,
             current_price     = $1,
             position_size_usd = $2
         WHERE id = $3`, [price, sizeUsd, positionId]);
            console.log(`[PriceService] Entry recorded: ${exports.RWA_TOKEN_MINTS[assetMint] || assetMint.slice(0, 8)} | $${price.toFixed(4)} | ${tokenAmount} tokens | $${sizeUsd.toFixed(2)}`);
        }
        catch (err) {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    async recordClosePrice(positionId, assetMint, tokenAmount) {
        try {
            const price = await jupiterPriceService_1.jupiterPriceService.getPrice(assetMint);
            if (!price) {
         ...
         WHERE id = $2`, [price, positionId]);
            console.log(`[PriceService] Close recorded: ${exports.RWA_TOKEN_MINTS[assetMint] || assetMint.slice(0, 8)} | $${price.toFixed(4)} | $${closeValueUsd.toFixed(2)} value at close`);
        }
        catch (err) {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    async updateAllOpenPositionPrices() {
        const startTime = Date.now();
        const result = { updated: 0, skipped: 0, errors: 0, pricesFetched: {}, durationMs: 0 };
        try {
            // 1. Get all distinct mints that have open positions
            const mintRows = await (0, pool_1.query)(`SELECT DISTINCT asset_mint, asset
         FROM positions
         WHERE status = 'open'
           ...
           AND token_amount > 0`);
            ...
            const prices = await jupiterPriceService_1.jupiterPriceService.getPrices(mints);
            ...
            const openPositions = await (0, pool_1.query)(`SELECT id, wallet, asset, asset_mint, token_amount, position_size_usd, entry_price, current_price
         FROM positions
         WHERE status = 'open'
           AND asset_mint IS NOT NULL
           AND token_amount > 0`);
            ...
            for (const pos of openPositions) {
             ...
             WHERE id = $4`, [price, newSizeUsd, entryPrice, pos.id]);
                    result.updated++;
                }
                catch (err) {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    async backfillMissingEntryPrices() {
        console.log('[PriceService] Backfilling missing entry prices…');
        const positions = await (0, pool_1.query)(`SELECT id, asset, asset_mint, token_amount
       FROM positions
       WHERE entry_price IS NULL
        ...
        for (const pos of positions) {
            ...
            await (0, pool_1.execute)(`UPDATE positions
         SET entry_price = $1
         WHERE id = $2 AND entry_price IS NULL`, [price, pos.id]);
            fixed++;
        }
...
exports.PositionPriceService = PositionPriceService;
exports.positionPriceService = new PositionPriceService();
//# sourceMappingURL=positionPriceService.js.map