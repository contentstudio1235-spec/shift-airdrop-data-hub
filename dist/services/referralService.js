...
async function resolveReferralCode(code) {
    ...
    const upperCode = (0, referralConstants_1.normalizeReferralCode)(code);
    // 1. Check KOL whitelist
    const kolResult = await pool_1.pool.query(`SELECT wallet, custom_code, display_name, multiplier_bonus, multiplier_type, is_active
     FROM kol_whitelist
     WHERE UPPER(custom_code) = $1`, [upperCode]);
    ...
    const userResult = await pool_1.pool.query(`SELECT wallet FROM users WHERE UPPER(referral_code) = $1`, [upperCode]);
 ...
 */
async function registerWithReferral(wallet, refCode) {
    ...
    try {
        ...
        // Insert user
        await client.query(`INSERT INTO users (wallet, total_xp, referral_code, permanent_multiplier, dynamic_multiplier, created_at, updated_at)
       VALUES ($1, 0, $2, 1.0, 1.0, NOW(), NOW())
       ON CONFLICT (wallet) DO UPDATE SET
         referral_code = COALESCE(users.referral_code, EXCLUDED.referral_code),
         updated_at = NOW()`, [wallet, userCode]);
        let bonusApplied = false;
        let bonusMultiplier = 1.0;
        let bonusType = 'none';
        let referrerDisplayName = null;
        if (refCode) {
            const codeInfo = await resolveReferralCode(refCode);
            if (codeInfo && codeInfo.isActive && codeInfo.referrerWallet !== wallet) {
                // Check not already referred
                const existingRef = await client.query(`SELECT id FROM referrals WHERE referred_wallet = $1`, [wallet]);
             ...
             ON CONFLICT (referred_wallet) DO NOTHING`, [wallet, codeInfo.referrerWallet, codeInfo.code, codeInfo.isKol, bonusMultiplier, bonusType]);
               ...
               referred_by_code = $3
             WHERE wallet = $1`, [wallet, codeInfo.referrerWallet, codeInfo.code]);
                    // Award multiplier if bonus > 1
                    if (bonusMultiplier > 1.0 && bonusType !== 'none') {
                        try {
                            await applyReferralMultiplier(client, wallet, bonusMultiplier, bonusType, codeInfo.code);
                        }
                        catch (err) {
                            ...
                            await client.query(`UPDATE referrals SET bonus_applied = false WHERE referred_wallet = $1`, [wallet]);
                    ...
                    await client.query(`UPDATE referrals SET bonus_applied = true WHERE referred_wallet = $1`, [wallet]);
                    bonusApplied = true;
                }
            }
        }
        ...
        // Get queue position
        const rankResult = await client.query(`SELECT ROW_NUMBER() OVER (ORDER BY created_at ASC) as queue_position
 ...
 */
async function applyReferralMultiplier(client, wallet, multiplierBonus, multiplierType, sourceCode) {
    if (multiplierType === 'dynamic') {
        // Insert dynamic multiplier slot (no expiry for referral bonus)
        await client.query(`INSERT INTO user_dynamic_multipliers
         (wallet, multiplier_value, reason, source, source_id, is_active)
       VALUES ($1, $2, $3, 'kol_referral', $4, true)`, [wallet, multiplierBonus, `KOL referral bonus from code ${sourceCode}`, sourceCode]);
        // Recalculate effective dynamic multiplier (max of active slots)
        const maxResult = await client.query(`SELECT MAX(multiplier_value) as max_mult
       FROM user_dynamic_multipliers
       WHERE wallet = $1 AND is_active = true
         AND (expires_at IS NULL OR expires_at > NOW())`, [wallet]);
        const newDynamic = parseFloat(maxResult.rows[0]?.max_mult || multiplierBonus);
        const oldResult = await client.query(`SELECT dynamic_multiplier FROM users WHERE wallet = $1`, [wallet]);
        const oldValue = parseFloat(oldResult.rows[0]?.dynamic_multiplier || '1.0');
        await client.query(`UPDATE users SET dynamic_multiplier = $2, updated_at = NOW() WHERE wallet = $1`, [wallet, newDynamic]);
        await client.query(`INSERT INTO multiplier_log
         (wallet, multiplier_type, old_value, new_value, reason, source, source_id)
       VALUES ($1, 'dynamic', $2, $3, $4, 'kol_referral', $5)`, [wallet, oldValue, newDynamic, `Referral bonus from ${sourceCode}`, sourceCode]);
    }
    else if (multiplierType === 'permanent') {
        try {
            // Get current values
            const userResult = await client.query(`SELECT total_xp, permanent_multiplier FROM users WHERE wallet = $1`, [wallet]);
            ...
            if (newPermMult > oldPermMult) {
                ...
                await client.query(`SELECT 1 FROM users WHERE wallet = $1 FOR UPDATE`, [wallet]);
                await client.query(`UPDATE users SET
             permanent_multiplier = $2,
             total_xp = $3,
             updated_at = NOW()
           WHERE wallet = $1`, [wallet, newPermMult, newXp]);
                await client.query(`INSERT INTO multiplier_log
             (wallet, multiplier_type, old_value, new_value, reason, source, source_id, xp_before, xp_after)
           VALUES ($1, 'permanent', $2, $3, $4, 'kol_referral', $5, $6, $7)`, [wallet, oldPermMult, newPermMult, `Retroactive referral bonus from ${sourceCode}`, sourceCode, currentXp, newXp]);
            }
        }
        catch (err) {
 ...
 */
async function awardInviteBonus(client, referrerWallet, isKol, referredWallet) {
    try {
        // Use constants-based bonus amounts (CRITICAL: consistent throughout system)
        const bonusXp = isKol ? referralConstants_1.INVITE_BONUS_XP.KOL : referralConstants_1.INVITE_BONUS_XP.STANDARD;
        await client.query(`UPDATE users SET
         total_xp = total_xp + $2,
         invite_bonus_xp = invite_bonus_xp + $2,
         updated_at = NOW()
       WHERE wallet = $1`, [referrerWallet, bonusXp]);
...
// ── KOL Admin Functions ────────────────────────────────────────────────────
async function addKol(params) {
    const { wallet, customCode, displayName, multiplierBonus = 1.5, multiplierType = 'dynamic', notes, createdBy, } = params;
    // Ensure user row exists
    await pool_1.pool.query(`INSERT INTO users (wallet, total_xp, referral_code, created_at, updated_at)
     VALUES ($1, 0, $2, NOW(), NOW())
     ON CONFLICT (wallet) DO UPDATE SET referral_code = COALESCE(users.referral_code, EXCLUDED.referral_code)`, [wallet, walletToCode(wallet)]);
    const result = await pool_1.pool.query(`INSERT INTO kol_whitelist
     ...
     ON CONFLICT (wallet) DO UPDATE SET
       ...
       updated_at       = NOW()
     RETURNING *`, [wallet, customCode, displayName || null, multiplierBonus, multiplierType, notes || null, createdBy || null]);
...
async function updateKol(wallet, updates) {
    const fields = [];
    const values = [];
    let idx = 2;
    if (updates.customCode !== undefined) {
        fields.push(`custom_code = UPPER($${idx++})`);
        values.push(updates.customCode);
    }
    if (updates.displayName !== undefined) {
        fields.push(`display_name = $${idx++}`);
        values.push(updates.displayName);
    }
    if (updates.multiplierBonus !== undefined) {
        fields.push(`multiplier_bonus = $${idx++}`);
        values.push(updates.multiplierBonus);
    }
    if (updates.multiplierType !== undefined) {
        fields.push(`multiplier_type = $${idx++}`);
        values.push(updates.multiplierType);
    }
    ...
    const result = await pool_1.pool.query(`UPDATE kol_whitelist SET ${fields.join(', ')} WHERE wallet = $1 RETURNING *`, [wallet, ...values]);
...
exports.default = exports.referralService;
