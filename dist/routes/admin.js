...
router.get('/kol/:wallet/referrals', verifyAdminSecret, async (req, res) => {
    try {
       ...
       LIMIT $2 OFFSET $3`, [wallet, parseInt(limit) || 100, parseInt(offset) || 0]);
        const totalResult = await pool_1.pool.query(`SELECT COUNT(*) as count FROM referrals WHERE referrer_wallet = $1`, [wallet]);
        const totalReferrals = parseInt(totalResult.rows[0]?.count) || 0;
        const appliedCount = referralsResult.rows.filter((r) => r.bonus_applied).length;
        res.json({
            success: true,
            kol: {
                wallet: kol.wallet,
...
router.get('/referrals/config', verifyAdminSecret, async (req, res) => {
    try {
        const configResult = await pool_1.pool.query(`SELECT standard_bonus_xp, kol_bonus_xp FROM referral_config WHERE id = 1`);
...
router.patch('/referrals/config', verifyAdminSecret, async (req, res) => {
    try {
        ...
        if (standardInviteXp !== undefined) {
            await pool_1.pool.query(`UPDATE referral_config SET standard_bonus_xp = $1, updated_at = NOW() WHERE id = 1`, [standardInviteXp]);
        }
        if (kolInviteXp !== undefined) {
            await pool_1.pool.query(`UPDATE referral_config SET kol_bonus_xp = $1, updated_at = NOW() WHERE id = 1`, [kolInviteXp]);
        }
        // Log audit trail
        await adminAuditService_1.adminAuditService.log(adminWallet, 'referral_config_updated', 'referral_config', 'global', null, { standardInviteXp, kolInviteXp }, reason || 'Updated referral configuration');
        res.json({
            success: true,
