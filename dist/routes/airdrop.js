...
router.get('/user/:wallet', async (req, res) => {
    try {
        ...
        const userQuery = await pool_1.pool.query(`SELECT
       ...
       WHERE u.wallet = $1`, [wallet]);
        ...
        // Total members
        const totalQuery = await pool_1.pool.query('SELECT COUNT(*) as count FROM users');
        const totalMembers = parseInt(totalQuery.rows[0].count);
        // Loyalty points from SNAG + sync log for net social SP calculation
        const loyaltyPoints = await snagSyncService_1.snagSyncService.getUserPoints(wallet).catch(() => 0);
        const syncLog = await pool_1.pool.query('SELECT last_synced_xp FROM xp_sync_log WHERE wallet = $1', [wallet]);
        ...
        // Referral count
        const refCount = await pool_1.pool.query('SELECT COUNT(*) as count FROM referrals WHERE referrer_wallet = $1', [wallet]);
...
router.get('/ref/:code', async (req, res) => {
    try {
        ...
        const info = await (0, referralService_1.resolveReferralCode)(code);
        if (!info) {
