    ...
    `);
        ...
        try {
            ...
            for (const fix of toFix) {
                await client.query(`UPDATE positions SET xp_generated = $1 WHERE id = $2`, [fix.new_xp, fix.id]);
                fixLog.push({
                    wallet: fix.wallet,
                    position_id: fix.id,
                    asset: fix.asset,
            ...
            for (const wallet of affectedWallets) {
                const sumResult = await client.query(`SELECT COALESCE(SUM(xp_generated), 0) as total FROM positions WHERE wallet = $1`, [wallet]);
                const newTotal = parseFloat(sumResult.rows[0].total);
                await client.query(`UPDATE users SET total_xp = $1, updated_at = NOW() WHERE wallet = $2`, [newTotal, wallet]);
            }
...
fix();
//# sourceMappingURL=fix-multiplier-bug.js.map