...
function PointsBreakdown({ positionSp, socialSp, referralSp, finalSp }: {
  ...
  const rows = [
    ...
    { label: 'Referral SP', raw: referralSp, mult: '×0.5', weighted: Math.round(referralSp * 0.5),   color: 'var(--amber)',      sub: 'Commission from referrals'     },
  ];
    ...
    <div className="card">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
        <div>
          ...
          <div style={{ fontSize: 10, color: 'var(--text-mute)', fontFamily: 'var(--font-mono)' }}>(Pos×2) + (Social×1) + (Ref×0.5)</div>
        </div>
...
export default function RegisterContent() {
  ...
  const finalSp       = Math.round(positionSp * 2 + socialSp + referralSp * 0.5);
        ...
        <div className="page" style={{ paddingTop: 0 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 16 }}>
            {[
              ...
              { n: '03', title: 'Refer & Complete',    desc: 'Share your link and finish SNAG quests. Earn Social SP (×1) and Referral SP (×0.5 quality-gated).',           color: 'var(--purple)' },
            ].map(s => (
          ...
          <div className="card" style={{ background: 'linear-gradient(135deg,rgba(247,162,59,0.06),rgba(247,162,59,0.02))', border: '1px solid rgba(247,162,59,0.2)', position: 'relative' }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 14 }}>
              <div style={{ flex: 1, padding: '10px 12px', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)', borderRadius: 9 }}>
                ...
                <div style={{ fontSize: 10, color: 'var(--text-mute)', marginTop: 2 }}>weighted ×0.5 in Final SP</div>
              </div>
              <div style={{ flex: 1, padding: '10px 12px', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)', borderRadius: 9 }}>
