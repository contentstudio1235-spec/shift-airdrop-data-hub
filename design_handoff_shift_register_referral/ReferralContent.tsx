  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: 10, padding: '11px 16px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
      ...
      {[
        ...
        { label: 'Referral SP', weight: '×0.5', color: 'var(--amber)',      active: true  },
      ].map((p, i, arr) => (
    ...
    <div className="card">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>Earn {commissionRate}% of referred traders' Position SP</div>
        </div>
        {referralCode && (
    ...
    <div className="card">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {[
          ...
          { text: 'Referral SP weighted at 0.5× in Final SP formula', ok: null  },
        ].map((r, i) => (
...
export default function ReferralContent() {
  ...
  const totalReferred  = myStats?.totalReferred  ?? stats?.referralCount ?? 0;
  const activeRefs     = myStats?.activeReferrals ?? 0;
  const commEarned     = myStats?.totalXpEarned   ?? 0;
  const inactiveCount  = Math.max(0, totalReferred - activeRefs);
      ...
      <div style={{ background: 'linear-gradient(135deg,var(--bg-2),var(--bg-3))', border: '1px solid rgba(247,162,59,0.22)', borderRadius: 20, padding: '28px 32px', marginBottom: 14, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 10, position: 'relative' }}>
          {[
            ...
            { label: 'Commission Earned', value: `${commEarned.toLocaleString()} SP`, color: 'var(--amber)', sub: 'Ref SP (×0.5 weight)'      },
