...
export interface PnLData {
  unrealizedUsd: number;   // (current_price - price_at_open) × token_amount
  unrealizedPct: number;   // Percentage: ((current_price - price_at_open) / price_at_open) × 100
  realizedUsd: number;     // For closed positions: (price_at_close - price_at_open) × token_amount
  realizedPct: number;     // Percentage for closed positions
  totalPnL: number;        // unrealized + realized
  totalPnLPct: number;     // Overall percentage
}
...
export function calculatePositionPnL(
  ...
  const unrealizedUsd =
    (currentPrice - position.price_at_open) * tokenAmount;
  const unrealizedPct =
    ((currentPrice - position.price_at_open) / position.price_at_open) * 100;
  ...
  const totalPnLPct =
    totalPnL === 0 ? 0 : (totalPnL / (position.price_at_open * tokenAmount)) * 100;
 ...
 */
export function calculateDashboardPnL(
  positions: Position[],
  ...
  for (const position of positions) {
    ...
    if (pnl) {
      ...
      const initialValue =
        (position.price_at_open || 0) * (position.token_amount || 0);
 ...
 */
export function formatPnL(pnl: PnLData | null): {
  usd: string;
  pct: string;
  isGain: boolean;
} {
