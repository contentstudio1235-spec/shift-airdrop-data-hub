...
interface CacheEntry { expiresAt: number; data: LaunchThisWeekSnapshot; }
const cache = new Map<string, CacheEntry>();

/** Test helper — clears module-level cache. */
export function _clearLaunchThisWeekCache(): void { cache.clear(); }
...
function passesConfidenceGate(row: LaunchThisWeekChannel): { pass: boolean; reason?: string } {
  ...
  if (PAID_MEDIUMS.has(medium)) {
    threshold = GATE_PAID; metric = 'signups';
  } else if (KOL_MEDIUMS.has(medium)) {
    threshold = GATE_KOL; metric = 'holders';
  } else {
    threshold = GATE_OTHER; metric = 'signups';
  }
...
export async function getLaunchThisWeekSnapshot(): Promise<LaunchThisWeekSnapshot> {
  ...
  if (campaignRows.length === 0) {
    ...
    const snapshot: LaunchThisWeekSnapshot = {
  ...
  const attrRows = await query<{
    ...
    stitched: string;
    holders: string;
    htft_median_seconds: string | null;
  }>(`
  ...
  const coverageRow = await queryOne<{ total: string; attributed: string }>(`
    SELECT
  ...
  const channels: LaunchThisWeekChannel[] = campaignRows
    ...
    .map(c => {
      ...
      const cac = spendUSD !== null && holders >= GATE_HOLDERS_FOR_CAC
        ...
        : null;
      const row: LaunchThisWeekChannel = {
        source: c.utm_source,
      ...
      if (!gate.pass) row.gateReason = gate.reason;
      return row;
    });
  ...
  const ranked = channels
    .filter(c => c.gatePass)
    .sort((a, b) => {
      if (a.cac !== null && b.cac !== null) return a.cac - b.cac;
  ...
  const campaignsMissingSpend = campaignRows.reduce(
    (sum, r) => sum + toNumber(r.missing_spend_count), 0
  );
  const activeCampaignCount = campaignRows.reduce(
    (sum, r) => sum + toNumber(r.campaign_count), 0
  );
...
async function getWeekStartISO(): Promise<string> {
  const row = await queryOne<{ ws: string }>(
    `SELECT date_trunc('week', NOW())::date::text AS ws`
  );
