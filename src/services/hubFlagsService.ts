...
const VALID_STATUSES: ReadonlySet<HubFlagStatus> = new Set([
  'open',
  'triaging',
  'closed_correct',
  'closed_delayed',
...
export function isHubFlagStatus(input: unknown): input is HubFlagStatus {
  return typeof input === 'string' && VALID_STATUSES.has(input as HubFlagStatus);
}

// Triage transitions allowed via PATCH. Closed_fixed is reached only by the
// incident-resolution flow (we don't expose it on triage).
export type HubFlagTriageStatus =
  | 'triaging'
  | 'closed_correct'
  | 'closed_delayed';

const VALID_TRIAGE_STATUSES: ReadonlySet<HubFlagTriageStatus> = new Set([
  'triaging',
  'closed_correct',
  'closed_delayed',
]);

export function isHubFlagTriageStatus(input: unknown): input is HubFlagTriageStatus {
  return typeof input === 'string' && VALID_TRIAGE_STATUSES.has(input as HubFlagTriageStatus);
}
...
export async function fileFlag(input: FileFlagInput): Promise<HubFlag> {
  const row = await queryOne<HubFlag>(
    `
...
export async function listFlags(filter: ListFlagsFilter = {}): Promise<HubFlag[]> {
...
export async function triageFlag(id: number, input: TriageFlagInput): Promise<HubFlag | null> {
  return queryOne<HubFlag>(
    `
    UPDATE hub_flags
    SET status          = $2,
        triaged_at      = NOW(),
