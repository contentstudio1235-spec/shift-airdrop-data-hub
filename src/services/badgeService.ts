...
export class BadgeService {
   ...
   */
  async checkLongHauler(wallet: string): Promise<BadgeAward | null> {
    if (await this.hasBadge(wallet, 'long_hauler')) return null;
   ...
   */
  async checkTheBeliever(wallet: string): Promise<BadgeAward | null> {
    if (await this.hasBadge(wallet, 'the_believer')) return null;
    ...
    const position = await queryOne(
      `SELECT id FROM positions
       WHERE wallet = $1 AND status != 'filtered'
         AND opened_at <= $2 LIMIT 1`,
      [wallet, ogWindowClose]
    );
    if (position) {
  ...
  private static readonly BADGE_XP: Record<string, number> = {
    common:  100,
    rare:    150,
    epic:    200,
    legend:  300,
  ...
  async awardBadge(wallet: string, badgeName: BadgeName): Promise<void> {
    ...
    if ((inserted as any)?.rowCount > 0) {
      ...
      const def = await queryOne<{ rarity: string }>(
        `SELECT rarity FROM badge_definitions WHERE badge_name = $1`,
        [badgeName]
      );
